...
def combination(loc):
    location_mapping = {
        ...
        68: (locA, locD, locE, locF, locH),
        69: (locA, locB, locC, locD, locE, locF),
        70: (locA, locB, locC, locD, locE, locG),
        71: (locA, locB, locC, locD, locE, locH),
        72: (locA, locB, locC, locE, locF, locG),
        73: (locA, locB, locC, locE, locF, locH),
        74: (locA, locB, locD, locE, locF, locG),
        75: (locA, locB, locD, locE, locF, locH),
        76: (locA, locC, locD, locE, locF, locG),
        77: (locA, locC, locD, locE, locF, locH),
        78: (locA, locB, locC, locD, locE, locF, locG),
        79: (locA, locB, locC, locD, locE, locF, locH),
        80: (locA, locB, locC, locD, locE, locF, locG, locH)
    }
...
class Model(nn.Module):
    def __init__(self, num_layers=3, num_vertices=7, channel1=16,channel2=32,channel3=64, conv_dim=4, timesteps=30):
...
def fitness_function(x):
    ...
    print("timestep:", input_timesteps, "loc:", loc, "vertices:", vertices, "layers:", num_layers, "channel1:", a, "channel2:", b, "channel3:", c, "conv:", conv_dim)
    channels = vertices - 1
    ...
    model = Model(num_layers=num_layers, num_vertices=vertices, channel1=a, channel2=b, channel3=c, conv_dim=conv_dim, timesteps=input_timesteps)
    dev1 = torch.device("cuda:0")
    dev2 = torch.device("cuda:1")
    model = model.double().to(dev1)

    if torch.cuda.device_count() > 1:
        model = DataParallel(model, device_ids=[dev1, dev2])  # Use specified GPU devices

    writer = SummaryWriter()
    train_dl, valid_dl = train_dl, valid_dl
    loss_func = F.l1_loss
    opt = optim.Adam(model.parameters())

    scaler = torch.amp.GradScaler('cuda')  # Automatic Mixed Precision (AMP)
    best_mean_valid_loss = 1e4
    r2score = R2Score(num_outputs=1, adjusted=0, multioutput='uniform_average').to(dev)
    
    for epoch in range(epochs):
        model.train()
        for xb, yb in train_dl:
            xb = xb[:, :, :input_timesteps].to(dev1)  # Move input to GPU and truncate input timeseries
            yb = yb.unsqueeze(1).to(dev1)

            with torch.amp.autocast('cuda'):  # Use mixed precision training
                pred = model(xb)
        ...
        model.eval()
        with torch.no_grad(), torch.amp.autocast('cuda'):  # Use mixed precision evaluation
            valid_loss = 0.0
            valid_num = 0
            valid_rmse = 0.0
            ...
            for xb, yb in valid_dl:
                ...
                batch_valid_loss = loss_func(model(xb), yb)
                valid_loss += batch_valid_loss.item()
                
                batch_rmse = torch.sqrt(torch.mean((model(xb)-yb)**2))
                valid_rmse += batch_rmse.item()
            ...
            r2score.reset()  # Reset the R2 Score metric
            for xb, yb in valid_dl:
                xb = xb[:, :, :input_timesteps].to(dev)
                yb = yb.unsqueeze(1).to(dev)
                pred = model(xb)
                r2score.update(pred, yb)  # Update R2Score metric with predictions and ground truth
                r2=r2score.compute()
                
            if mean_valid_loss < best_mean_valid_loss:
                torch.save(model.state_dict(), "trained_models/cppsocauchy_phdry_" + date_time_string + ".pt")
    ...
    time = t2_0 - t1_0
    print('MAE:', best_mean_valid_loss,'RMSE:', mean_valid_rmse, 'time:', time, "file:", "pso_phildry_" + date_time_string)
...
problem_dict1 = {
    ...
    "bounds":FloatVar(lb= [24, 0, 1, 1, 1, 1, 1],ub=[48.99, 80.99, 3.99, 64.99, 64.99, 64.99, 24.99]),
    "minmax": "min",
    "log_to": "file",
    "log_file": "logs/ppso_dry.log",         # Default value = "mealpy.log"
    "name": "GCNN-Dry-PPSO"
}
epoch = 50
pop_size = 21
...
print("GPU computation time for PPSO: " + str(t2_1-t1_1))
